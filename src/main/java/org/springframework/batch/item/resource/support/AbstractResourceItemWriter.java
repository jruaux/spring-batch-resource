...
public abstract class AbstractResourceItemWriter<T> extends AbstractItemStreamItemWriter<T>
     ...
     *
     * @param newEncoding {@link String} containing the encoding to be used for the writer.
     */
     ...
     * true, so no appending except on restart. If set to false and {@link #setAppendAllowed(boolean) appendAllowed} is also
     * false then there will be an exception when the stream is opened to prevent existing data being potentially corrupted.
     * 
     ...
     * {@link #setShouldDeleteIfExists(boolean) shouldDeleteIfExists} is automatically set to false, so that flag should not be
     * set explicitly. Defaults value is false.
     * 
    ...
    /**
     * Flag to indicate that the target file should be deleted if no lines have been written (other than header and footer) on
     ...
     * {@link ItemStream} call to update. Setting this to false means that it will always start at the beginning on a restart.
     * 
    ...
    /**
     * headerCallback will be called before writing the first item to file. Newline will be automatically appended after the
     * header is written.
     *
    ...
    /**
     * Writes out a string followed by a "new line", where the format of the new line separator is determined by the underlying
     * operating system.
     * 
    ...
    /**
     * Write out a string of items followed by a "new line", where the format of the new line separator is determined by the
    ...
    protected class OutputState {
        ...
        /**
         * Returns the buffered writer opened to the beginning of the file specified by the absolute path name contained in
         * absoluteFileName.
         */
        ...
        /**
         * Checks (on setState) to make sure that the current output file's size is not smaller than the last saved commit
         * point. If it is, then the file has been damaged in some way and whole task must be started over again from the
         * beginning.
         * 
